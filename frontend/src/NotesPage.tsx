...
export default function NotesPage() {
  ...
  function groupedNotes() {
    ...
    for (const note of notes) {
      const key = note.topic_name || "Unsorted";
        ...
        <div className="mt-6 rounded-2xl border-2 border-slate-200 bg-white p-4">
          {loading ? (
            ...
            <div className="text-sm text-slate-600">
              No notes uploaded yet. Click <b>+</b> to upload your first PDF.
            </div>
          ) : (
            <div className="space-y-6">
              {groupedNotes().map(([topicName, items]) => (
                <div key={topicName}>
...
>
  <a
    ...
    title="Open file"
  >
    <div className="truncate font-medium">{n.filename}</div>
      ...
      {showUpload && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 px-4">
          <div className="w-full max-w-lg rounded-2xl border-2 border-slate-200 bg-white p-4">
            <div className="mb-3 text-lg font-semibold">Upload Notes</div>
              ...
              {selectedTopicId === "new" && (
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. Electricity, Mechanics, Waves…"
                  />
                </div>
              )}
