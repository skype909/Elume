...
type Teacher = { id: number; email: string; first_name?: string | null; last_name?: string | null; is_active: boolean; role: "teacher" | "school_admin" };
...
export default function SchoolAdminPage() {
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-emerald-50 px-4 py-7 sm:px-6 lg:px-8">
          ...
          <section className="mt-7 grid gap-7 xl:grid-cols-[1.45fr_0.9fr]">
            <div className="space-y-7">
              <Panel title="Teachers" subtitle="Teacher data and classes are preserved when access is disabled.">
                {teachers.length === 0 ? <Empty text="No teachers have joined this school yet." /> : <div className="divide-y divide-slate-100">{teachers.map((teacher) => <div key={teacher.id} className="flex flex-col gap-3 py-4 sm:flex-row sm:items-center sm:justify-between"><div><div className="font-bold text-slate-900">{`${teacher.first_name || ""} ${teacher.last_name || ""}`.trim() || "Teacher"}{teacher.role === "school_admin" && <span className="ml-2 rounded-full bg-violet-100 px-2 py-0.5 text-xs font-bold text-violet-700">School Admin</span>}</div><div className="text-sm text-slate-600">{teacher.email}</div></div><div className="flex items-center gap-3"><StatusBadge active={teacher.is_active} label={teacher.is_active ? "Active" : "Disabled"} />{teacher.role === "teacher" && <button type="button" disabled={actionKey !== null} onClick={() => teacher.is_active ? setConfirm({ kind: "disable", id: teacher.id, label: teacher.email }) : teacherAction(teacher, "reactivate")} className={`rounded-xl px-4 py-2 text-sm font-bold transition disabled:opacity-50 ${teacher.is_active ? "border border-red-200 bg-red-50 text-red-700 hover:bg-red-100" : "bg-emerald-600 text-white hover:bg-emerald-700"}`}>{actionKey === `reactivate-${teacher.id}` ? "Reactivating…" : teacher.is_active ? "Disable" : "Reactivate"}</button>}</div></div>)}</div>}
              </Panel>
              ...
              <Panel title="Departments" subtitle="Teachers and School Admins can belong to more than one department. Sharing never changes ownership of original resources.">
      ...
      {departmentMembersOpen && <Modal title={`${departmentMembersOpen.name} members`} onClose={() => !actionKey && setDepartmentMembersOpen(null)}><p className="mb-3 text-sm text-slate-600">Select active teachers or School Admins. Members can appear in several departments.</p><div className="max-h-72 space-y-2 overflow-auto">{teachers.filter((teacher) => teacher.is_active).map((teacher) => { const checked = departmentMembersOpen.members.some((member) => member.id === teacher.id); return <label key={teacher.id} className="flex cursor-pointer items-center gap-3 rounded-xl border border-slate-200 p-3"><input type="checkbox" checked={checked} onChange={() => setDepartmentMembersOpen((current) => current ? { ...current, members: checked ? current.members.filter((member) => member.id !== teacher.id) : [...current.members, teacher] } : current)} /><span className="text-sm font-semibold text-slate-800">{`${teacher.first_name || ""} ${teacher.last_name || ""}`.trim() || teacher.email}{teacher.role === "school_admin" ? " (School Admin)" : ""}</span></label>; })}</div><div className="mt-5 flex justify-end gap-3"><button type="button" onClick={() => setDepartmentMembersOpen(null)} className="rounded-xl px-3 py-2 font-bold text-slate-600">Cancel</button><button type="button" onClick={() => void saveDepartmentMembers()} className="rounded-xl bg-emerald-600 px-4 py-2 font-bold text-white">Save members</button></div></Modal>}
    </main>
  );
}
