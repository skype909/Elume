    ...
    useEffect(() => {
        ...
        async function load() {
            try {
                ...
                const json = await apiFetch(`${API_BASE}/classes/${id}/students/${studentId}/report-data`);
                if (!cancelled) setData(json);
            } catch (error: unknown) {
                if (!cancelled) {
                    setError(userFacingError(error, "We couldn’t load this student report just yet. Give it another try."));
                }
            } finally {
            ...
            <div className="min-h-screen bg-emerald-50 p-6 text-slate-900 md:p-8">
                <div className="mx-auto max-w-3xl pt-12">
                    <InlineNotice
                        ...
                        message={error || "We couldn’t load this student report just yet. Give it another try."}
