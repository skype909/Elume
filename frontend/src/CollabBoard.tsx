        ...
        >
          <svg className="h-full w-full overflow-visible">
            <rect
              ...
              rx="14"
              ry="14"
              width={Math.max(20, obj.w - 4)}
              height={Math.max(20, obj.h - 16)}
              fill="white"
              stroke="#0f172a"
              strokeWidth="2"
            />
            <polygon
              points={`${obj.w * 0.22},${obj.h - 14} ${obj.w * 0.32},${obj.h - 14} ${obj.w * 0.18},${obj.h}`}
              fill="white"
              stroke="#0f172a"
              strokeWidth="2"
            />
          </svg>
          <div className="absolute inset-0 flex items-center justify-center px-4 pb-3 text-sm font-semibold text-slate-700">
            {obj.text || "Speech..."}
          </div>
          {isSelected && editable && (
            <div
